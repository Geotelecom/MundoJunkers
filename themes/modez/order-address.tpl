...
{else}
	...
	<h1 class="step-num account">{l s='Addresses'}
	{if $is_logged AND !$is_guest}
		<span><a href="{$link->getPageLink('index', true, NULL, 'mylogout')|escape:'html':'UTF-8'}" id="openLoginFormBlock">{l s='Cerrar sesion'} </a></span>
	{/if}
	</h1>

	{if $is_logged AND !$is_guest}
	<p class="userlogin">{$cookie->customer_firstname} {$cookie->customer_lastname}</p>
	{/if} 
...
<div class="addresses clearfix">
	<div class="row">
		 <div class="col-xs-12 col-sm-12 unvisible">
	...
	<div class="row">
		<div class="col-xs-12 col-sm-12"{if $cart->isVirtualCart()} style="display:none;"{/if}>
			<ul class="address item box-form" id="address_delivery">
			</ul>
		</div>
		<div class="col-xs-12 col-sm-12">
			<ul class="address alternate_item{if $cart->isVirtualCart()} full_width{/if} box-form" id="address_invoice">
			</ul>
		</div>
	</div> <!-- end row -->
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button btn btn-default continue_shoping">
