...
$(document).ready(function(){
	...
	if ($(window).width()<768) $('#btn-account').show();
	...
	if ((typeof isLogged == 'undefined' || !isLogged) || (typeof isGuest !== 'undefined' && isGuest))
	{
		if (guestCheckoutEnabled && !isLogged)
		{
			...
			// modificació de usuari registre/invitat
			...
			if (typeof bindUniform !=='undefined')
				bindUniform();		
			// --> fi modificació de usuari registre/invitat

			$(document).on('click', '#openChoise',function(e){

				$('#opc_account_choice, #opc_invoice_address').show();
				if ($('#opc_account_form').is(':visible')) $('#opc_account_form').hide();

				$('.openChoise').hide();
				//$('#submitGuestAccount').hide();

				if ($(window).width()>768) $('#submitGuestAccount span').html('Registro <i class="icon-chevron-right right"></i>');
			...
			$(document).on('click', '#opc_guestCheckout', function(e){
				...
				$('.email_param').removeClass('col-md-7');
		...
		$(document).on('click', '#submitAccount, #submitGuestAccount', function(e){
			...
			if (isLogged)	{
				//var url = $("input:radio[name=delivery_pay]:checked").val();
				//window.location = url;

				var  i = $("input:radio[name=delivery_pay]:checked").data('button');
				$('.paymentlink_'+i)[0].click();
				return;
			}
	...
	if (isLogged)	{
		...
		$('#submitGuestAccount span').html('Comprar <i class="icon-chevron-right right"></i>');
...
function bindInputs()
{
	...
	$('#cgv').on('click', function(e){
		...
		//if($('#opc_account_choice').is(":hidden")){
			if ($('#cgv:checked').length !== 0){
				checked = 1;
				$('#submitGuestAccount').show();
			} else {
