...
<div class="capcaleracontact">
    <div class="asun"></div>
    <div class="content">
        <h1>{l s='Soy Asun, ¿Te Ayudo?'}</h1>
        <p>{l s='Si te puedo ayudar de alguna forma, por favor, llámanos.'}</p>
        <p class="telf">{l s='977 070 001'} <span>/</span> {l s='691 097 366'} <small>(también whatsapp)</small></p>
    </div>
</div>
{/if}
...
{else}
	...
	<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="contact-form-box" enctype="multipart/form-data">
        ...
        {else}
            <h3>{l s='Si prefieres déjanos tu número de teléfono y te llamamos'} <span>{l s='GRATIS'}</span></h3>
        {/if}
        <div class="clearfix">
...
{*}
                    <p class="form-group">
                    <label for="code">{l s='Código'}</label>
                        <img src="{$base_dir_ssl}captcha/securimage_show.php?sid={Tools::passwdGen(16)|md5}" id="codi" alt="{l s='código de seguridad'}"><br/>
                          <small class="recarga"><span onclick="document.getElementById('codi').src = '{$base_dir_ssl}captcha/securimage_show.php?sid=' + Math.random(); this.blur(); return false">{l s='click para recargar el código'}</span></small>            
                    </p>
                    <p class="form-group">
                    <label for="code">{l s='Escriba el código de seguridad'}</label>
