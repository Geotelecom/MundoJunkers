...
{else}
		...
		<table id="cart_summary" class="table table-bordered">
						...
						<tr class="cart_total_price">
							<td rowspan="{3+$total_discounts_num+$use_show_taxes+$total_wrapping_taxes_num}" colspan="2" id="cart_voucher" class="cart_voucher">
								{if $voucherAllowed}
									...
									{/if}
									<span class="disponescupon">{l s='¿Dispones de un cupón de descuento?'} <span onClick="$('#voucher').fadeToggle();">{l s='Úsalo haciendo click aquí'}</span></span>
			...
			<tbody>
				...
				{foreach $products as $product}
					...
					{include file="$tpl_dir./shopping-cart-product-line-full.tpl" productLast=$product@last productFirst=$product@first}
					...
					{if isset($customizedDatas.$productId.$productAttributeId)}
						...
						{if $product.quantity-$quantityDisplayed > 0}{include file="$tpl_dir./shopping-cart-product-line-full.tpl" productLast=$product@last productFirst=$product@first}{/if}
					{/if}
				{/foreach}
	...
	<p class="cart_navigation clearfix">
		{*if !$opc*}
			...
			<a
				href="{if $back}{$link->getPageLink('order-opc', true, NULL, '&amp;onepagecheckout=0&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order-opc', true, NULL, '&amp;onepagecheckout=1')|escape:'html':'UTF-8'}{/if}"
