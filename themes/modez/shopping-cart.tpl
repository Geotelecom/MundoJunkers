...
<h1 class="step-num summary">{l s='Shopping-cart summary'}
...
{else}
	...
	{if isset($lastProductAdded) AND $lastProductAdded}
		<div class="cart_last_product">
			...
			<a class="cart_last_product_img" href="{$link->getProductLink($lastProductAdded.id_product, $lastProductAdded.link_rewrite, $lastProductAdded.category, null, null, $lastProductAdded.id_shop)|escape:'html':'UTF-8'}">
				<img src="{$link->getImageLink($lastProductAdded.link_rewrite, $lastProductAdded.id_image, 'small_default')|escape:'html':'UTF-8'}" alt="{$lastProductAdded.name|escape:'html':'UTF-8'}" title="{$lastProductAdded.name|escape:'html':'UTF-8'}"/>
			</a>
			...
			<tfoot>
					...
					{else}
						...
						<tr>
							<td rowspan="1" colspan="3" id="cart_voucher" class="cart_voucher">
									<h1 class="step-num voucher"><a href="#" class="vouchers_link">{l s='Vouchers'}</a></h1>
				...
				{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
					<tr class="cart_total_delivery" style="{if !isset($carrier->id) || is_null($carrier->id)}display:none;{/if}">
						<td colspan="2" class="text-right">{l s='Shipping'}</td>
											...
											<div class="customizationUploaded">
												<ul class="customizationUploaded">
													{foreach $custom_data as $picture}
														<li><img src="{$pic_dir}{$picture.value}_small" alt="Imagen" class="customizationUploaded" /></li>
													{/foreach}
												</ul>
											</div>
			...
			{else}
				{foreach from=$formattedAddresses key=k item=address}
					<div class="col-xs-12 col-sm-6"{if $k == 'delivery' && !$have_non_virtual_products} style="display: none;"{/if}>
						<ul class="address {if $address@last}last_item{elseif $address@first}first_item{/if} {if $address@index % 2}alternate_item{else}item{/if} box-form">
							<li>
	...
	<p class="cart_navigation clearfix">
		...
		{*<a
			...
			class="button btn btn-default continue_shoping"
		...
		<div class="submit opc-add-save clearfix" id="buttonBuy">
			<button class="button btn btn-default button-medium  pull-left unvisible" id="btn-cart-back" ><span>  <i class="icon-chevron-left left"></i> {l s='Back'}</span></button>								
			{*}<div class="bg-secure">
				<p>{l s='Payment 100% insurance'}</p>
			</div>
			<div class="bg-customer"> 
				<p>{l s='Customers satisfied'}</p>
			</div>{*}
			<img src="{$img_dir}pago-seguro.png" alt="{l s='Pago seguro'}" style="float:left;"/>			
            <button type="submit" name="submitAccount" id="submitAccount" class="btn btn-default button button-medium exclusive pull-right unvisible"><span>{l s='Buy'}<i class="icon-chevron-right right"></i></span></button>            
		</div>	
