...
{else}
	<h1 class="step-num pay">{l s='Please choose your payment method'}</h1>
{/if}
		...
		<div class="paiement_block">
			<div id="HOOK_TOP_PAYMENT">{$HOOK_TOP_PAYMENT}</div>
				{if $HOOK_PAYMENT}
																...
																<div class="customizationUploaded">
																	<ul class="customizationUploaded">
																		{foreach from=$datas item='picture'}
																			<li>
																				<img src="{$pic_dir}{$picture.value}_small" alt="Imagen" class="customizationUploaded" />
																			</li>
																		{/foreach}
																	</ul>
																</div>
					...
					<div id="HOOK_PAYMENT" class="unvisible">
						{$HOOK_PAYMENT}						
					</div>
				...
				{if !$opc}
					<p class="cart_navigation clearfix">
						<a href="{$link->getPageLink('order', true, NULL, "step=2")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button btn btn-default continue_shoping">
			...
			//modificació afegir radio button a metodes pagament
	
				$('#HOOK_PAYMENT .payment_module').each(function (i) {
					...
					//$(this).append( '<div class="radio_param"><input id="delivery_pay" name="delivery_pay"  type="radio" '+checked+' value="'+link+'"></div><div class="radio_text_param">'+text+'</div>' );
					var imgs = '';
					if ($(this).find('a').hasClass('redsys'))
					{
						imgs += '<p class="imgs">';
						imgs += '<img src="{/literal}{$img_dir}{literal}paymentlogo_visa.png" alt="visa" title="visa" />';
						imgs += '<img src="{/literal}{$img_dir}{literal}paymentlogo_mastercard.png" alt="mastercard" title="mastercard" />';
						imgs += '</p>'
					}
				

					$(this).append( '<div class="radio_param"><input data-button='+i+' id="delivery_pay" name="delivery_pay"  type="radio" '+checked+'></div><div class="radio_text_param">'+imgs+text+'</div>' );


					//$(this).find('a').remove();
					$(this).find('a').hide();
					$(this).find('a').addClass('paymentlink_'+i);		

				});

				$('#HOOK_PAYMENT').show();
			});
			//--> fi modificació afegir radio button a metodes pagament	
