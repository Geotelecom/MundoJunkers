...
class ProductsCategory extends Module
{
	...
	public function hookProductFooter($params)
	{
		...
		{*/
			...
			// Get infos
			$category_products = $category->getProducts($this->context->language->id, 1, 12); /* 100 products max. */
			...
			if (is_array($category_products) && count($category_products))
			{
				...
				if ($nb_category_products > 8)
				{
					$category_products = array_slice($category_products, $middle_position - 4, 8, true);
					$middle_position = 4;
				}
			}
