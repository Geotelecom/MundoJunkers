...
class OnePageCheckoutPS extends OnePageCheckoutPSCore
{
    ...
    protected $configure_vars = array(
        ...
        array('name' => 'OPC_AUTO_ADDRESS_GEONAMES', 'default_value' => 1, 'is_html' => false, 'is_bool' => true),
    ...
    public function install()
    {
        ...
        $modules_without_popup .= 'multisafepaymastercard';
        ...
        $sql_country .= ' SET fs.default_value = \''.(int)Configuration::get('PS_COUNTRY_DEFAULT').'\'';
        ...
        if (is_writable($file)) {
            ...
            foreach ($data_translation as $key => $value) {
                foreach ($value as $data) {
                    ...
                    $value_translation = $data['value_translation'];

                    if (!empty($value_translation)) {
            ...
            'show_link_continue_shopping'  => array(
                ...
                'depends'  => array(
                    'link_continue_shopping' => array(
                        ...
                        'hidden_on'   => !$this->config_vars['OPC_SHOW_LINK_CONTINUE_SHOPPING'],
            ...
            'confirmation_button_float' => array(
                ...
                'depends'  => array(
                    'background_button_footer' => array(
                        ...
                        'color'  => true,
                        'hidden_on' => !$this->config_vars['OPC_CONFIRMATION_BUTTON_FLOAT'],
            ...
            'payment'         => array(
                ...
                'sub_tab' => array(
                    'popup_window' => array(
                        ...
                        'href'  => 'popup_window',
                        'icon'  => 'fa-pts-external-link',
                    ),
                    'pay_methods'  => array(
                        'label' => $this->l('Pay methods'),
                        'href'  => 'pay_methods',
                        'icon'  => 'fa-pts-credit-card',
                    ),
                    'ship_pay'     => array(
...
//			$default_value = Tools::substr($default_value, 0, $field_class->size);
    ...
    public function getRequiredFieldList()
    {
        ...
        $actions = array(
            'edit'   => array(
                ...
                'icon'         => 'fa-pts fa-pts-edit nohover',
                'title'        => $this->l('Edit'),
                'tooltip'      => $this->l('Edit'),
            ),
            'remove' => array(
                ...
                'icon'         => 'fa-pts fa-pts-times nohover',
                'title'        => '',
    ...
    public function hookHeader()
    {
        ...
        /*if (!$this->context->customer->isLogged()) {
            echo '<style>
                #cart_block, .cart_block{display: none !important;}
        ...
        if (isset(Context::getContext()->controller->php_self)
            ...
            foreach ($this->context->controller->js_files as $key => $js) {
                ...
                } elseif ($js == '/modules/leotempcp/bootstrap/js/bootstrap.js') {
                    unset($this->context->controller->js_files[$key]);
                }
                
                if ($this->config_vars['OPC_AUTO_ADDRESS_GEONAMES']) {
                    if (strpos($js, 'jquery.autocomplete.js')) {
                        unset($this->context->controller->js_files[$key]);
                    } elseif (strpos($js, 'blocksearch.js')) {
                        unset($this->context->controller->js_files[$key]);
                    }
                }
            ...
            case 'correos':
                ...
                if ($correos) {
                    ...
                    if (version_compare($correos->version, '3.0.0', '<=')) {
                        ...
                        //compatibilidad con version 4.0.1
                        ...
                        if (in_array($carrier['instance']->id, $correos_carriers['carriers_office'])) {
                            $offices = CorreosCommon::getOfficies($address->postcode, $carrier['instance']->id);
                            $offices = $this->jsonDecode($offices);

                            if ($offices) {
                                foreach ($offices as $office) {
                                    if ($office->selected == '1') {
...
//					}
    ...
    public function loadAddress()
    {
        ...
        if (!isset($this->context->cookie->opc_suggest_address)) {
            if ($this->context->customer->isLogged()) {
                ...
                if ($result) {
                    $id_address_delivery = $result[0]['id_address_delivery'];
                    $id_address_invoice = $result[0]['id_address_invoice'];
    ...
    public function loadCarrier()
    {
        ...
        $id_country          = Tools::getValue('id_country', null);
        $id_state            = Tools::getValue('id_state', null);
...
//                    }
                        ...
                        if (!$module_instance->currencies
                            ...
                            if ($module['name'] == 'twenga') {
                                ...
                                if (count($array_payment_need_register) > 0) {
                                    if (in_array($module['name'], $array_payment_need_register)) {
    ...
    public function loadReview()
    {
        ...
        if ($this->isModuleActive('customtextdesign')
            && file_exists(dirname(__FILE__).'/../customtextdesign/views/js/cart.js')
        ) {
            array_push($js_files, $this->getUrlStore().$this->context->shop->getBaseURI().'modules/customtextdesign/views/js/cart.js');
        }
    ...
    public function addModulesExtraFee()
    {
        ...
        if ($module !== false) {
            ...
            } elseif (method_exists($module, 'getFee') && $module->author == 'www.Presta-apps.com') {
