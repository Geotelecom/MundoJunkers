...
{else}
    {assign var='register_customer' value=((isset($smarty.get.rc) && $smarty.get.rc == 1 && !$IS_LOGGED) || ($show_authentication && !$IS_LOGGED && isset($no_products) && $no_products > 0)) ? true : false}
            ...
            {foreach from=$countries item='country'}
                ...
                {if isset($country.states) && $country.contains_states}
                    ...
                    {foreach from=$country.states item='state' name='states'}
                        {if $state.active eq 1}countries[{$country.id_country|intval}].push({ldelim}id : '{$state.id_state|intval}', name:'{$state.name|escape:'html':'UTF-8'|trim}', iso_code:'{$state.iso_code|escape:'html':'UTF-8'|trim}'{rdelim});{/if}
                    {/foreach}
                {/if}
