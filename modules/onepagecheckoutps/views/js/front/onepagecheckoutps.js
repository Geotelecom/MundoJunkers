...
var Fronted = {
    launch: function(){
        ...
        if ($('div#onepagecheckoutps').hasClass('rc')) {
            $('div#onepagecheckoutps #opc_show_login').trigger('click');
        }
...
var Address = {
    ...
    initPostCodeGeonames: function(params){
        ...
        if (OnePageCheckoutPS.CONFIGS.OPC_AUTO_ADDRESS_GEONAMES/* && !OnePageCheckoutPS.CONFIGS.OPC_SHOW_LIST_CITIES_GEONAMES*/){
    ...
    getCitiesByState: function(params){
        ...
        if (OnePageCheckoutPS.CONFIGS.OPC_SHOW_LIST_CITIES_GEONAMES) {
            ...
            var name_state = $id_state.find('option:selected').data('text');
            ...
            if ($id_state.length > 0 && !$.isEmpty(name_state)) {
                ...
                jeoquery.getGeoNames(
                  ...
                  function(data){
                    $('#onepagecheckoutps_step_one #'+param.object+'_city').hide();
                    ...
                    if ($city_list.length <= 0) {
                        $city_list = $('<select/>')
                            ...
                            .on('change', function(event){
                                $('#onepagecheckoutps_step_one #'+param.object+'_city').val($(event.currentTarget).val()).trigger('blur');
                            }
                        );
                    } else {
                        $city_list.html('').show();
                ...
                $('#onepagecheckoutps_step_one #'+param.object+'_city').val('').show();
            }
        }
    },
                            ...
                            $.each(json.addresses, function(i, address){
                                ...
                                if (typeof $.totalStorageOPC !== typeof undefined) {
                                    if ($.totalStorageOPC('id_address_delivery')) {
                                        if (address.id_address == $.totalStorageOPC('id_address_delivery')){
                                            $option.attr('selected', 'true');
                                        }
                                    } else {
                            ...
                            $.each(json.addresses, function(i, address){
                                ...
                                if (typeof $.totalStorageOPC !== typeof undefined) {
                                    if ($.totalStorageOPC('id_address_invoice')) {
                                        if (address.id_address == $.totalStorageOPC('id_address_invoice')){
                                            $option.attr('selected', 'true');
                                        }
                                    } else {
    ...
    load: function(params){
        ...
        var callback = function(){
            ...
            if (OnePageCheckoutPS.IS_VIRTUAL_CART && !loaded){
                if (OnePageCheckoutPS.SHOW_DELIVERY_VIRTUAL && $('div#onepagecheckoutps #delivery_id_country').length > 0){
                    ...
                    if(!json.hasError && (!$.isEmpty(json.customer.id) || !$.isEmpty(json.address_delivery.id) || !$.isEmpty(json.address_invoice.id))){
                        ...
                        $('div#onepagecheckoutps #onepagecheckoutps_step_one').find(object_load).each(function(i, field){
                            ...
                            if (name == 'id_country') {
                                ...
                                Address.updateState({object: object, id_state_default: id_state});
                            }
    ...
    fillInAddress: function(address, autocomplete) {
        Address.componentForm = {
            ...
            street_number: {index: 2, type: 'short_name', field: address + '_address1'},
            route: {index: 3, type: 'long_name', field: address + '_address1'},
            administrative_area_level_1: {index: 4, type: 'select', field: address + '_id_state'},
            administrative_area_level_2: {index: 5, type: 'select', field: address + '_id_state'},
            administrative_area_level_3: {index: 6, type: 'select', field: address + '_id_state'},
            country: {index: 7, type: 'select', field: address + '_id_country'}
        };
                ...
                if (component.type === 'select') {
                    if (component.name === 'country') {
                        ...
                        Address.callBackState = function() {
                            ...
                            if (typeof $('#' + address + '_id_state option[data-iso-code="' + component.short_name + '"]')[0] !== typeof undefined) {
                                $('#' + address + '_id_state option[data-iso-code="' + component.short_name + '"]').prop('selected', true);
                                return true;
                            } else if (typeof $('#' + address + '_id_state option[data-text="' + component.value + '"]')[0] !== typeof undefined) {
                                state_name = component.value;
                            } else if (components[5].name == 'administrative_area_level_2' && typeof $('#' + address + '_id_state option[data-text="' + components[5].value + '"]')[0] !== typeof undefined) {
                                state_name = components[5].value;
                            } else if (components[6] == 'administrative_area_level_3' && typeof $('#' + address + '_id_state option[data-text="' + components[6].value + '"]')[0] !== typeof undefined) {
                                state_name = components[6].value;
                            }
                    ...
                    tmp_value = $('#' + component.field).val();

                    if (component.field == (address + '_address1') && !$.isEmpty(tmp_value)) {
                        $('#' + component.field).val(tmp_value + ' ' + component.value);
                    } else {
    ...
    updateState: function(params) {
        ...
        if (!$.isEmpty(param.object)) {
			...
			if ($id_country.length <= 0 && $.isEmpty(id_country))
    ...
    displayPopupModule_correos: function(id_carrier){
        ...
        if($('div#onepagecheckoutps #onepagecheckoutps_step_two #shipping_container #correos_content').length > 0){
            ...
            var callback = function(){
                ...
                $('#opc_correos .btn_correos').on('click', function(){
                    $.totalStorageOPC('correos_postcode', $('#opc_correos #correos_postcode').val());
                });
            };
            var callback_close = function(){
                $('div#onepagecheckoutps #onepagecheckoutps_step_one #delivery_postcode').val($.totalStorageOPC('correos_postcode'));
...
var Payment = {
    ...
    getByCountry: function(params){
        ...
        var _json = {
            data: data,
            beforeSend: function() {
                if ($('#onepagecheckoutps_step_three #braintree-payment-form').length > 0) {
                    if (typeof $.totalStorageOPC !== typeof undefined) {
                        $.totalStorageOPC('braintree-card-number', $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-number').val());
                        $.totalStorageOPC('braintree-card-cvc', $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-cvc').val());
                        $.totalStorageOPC('braintree-card-expiry-month', $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-expiry-month').val());
                        $.totalStorageOPC('braintree-card-expiry-year', $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-expiry-year').val());
                    }
                }
            ...
            complete: function(){
                if ($('#onepagecheckoutps_step_three #braintree-payment-form').length > 0) {
                    if (typeof $.totalStorageOPC !== typeof undefined) {
                        $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-number').val($.totalStorageOPC('braintree-card-number'));
                        $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-cvc').val($.totalStorageOPC('braintree-card-cvc'));
                        $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-expiry-month').val($.totalStorageOPC('braintree-card-expiry-month'));
                        $('#onepagecheckoutps_step_three #braintree-payment-form #braintree-card-expiry-year').val($.totalStorageOPC('braintree-card-expiry-year'));
                    }
                }
...
//                });
    ...
    placeOrder: function(params){
        ...
        if(fields && AppOPC.is_valid_all_form){
            ...
            //support module correos
            if ($('#correos_mobile').length > 0) {
                $('#correos_mobile').val($('#onepagecheckoutps_step_one #delivery_phone_mobile').val());
            }
            if ($('#correos_email').length > 0) {
                $('#correos_email').val($('#onepagecheckoutps_step_one #customer_email').val());
            }
    ...
    //                                        window.location.hash = '#' + 'ad=' + data.id_address_delivery + '&ai=' + data.id_address_invoice;
                                            ...
                                            if (!OnePageCheckoutPS.OPC_SHOW_POPUP_PAYMENT || $.inArray(name_payment, arr_payments_without_popup) != -1 || $.strpos(url_payment, '?pm='+name_payment)){
    //                                            window.location.hash = '#' + 'ad=' + data.id_address_delivery + '&ai=' + data.id_address_invoice;
