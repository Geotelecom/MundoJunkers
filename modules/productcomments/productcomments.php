...
class ProductComments extends Module
{
	...
	public function __construct()
	{
		...
		// generate securekey
		if (!Configuration::get('productcomments_securekey'))
			Configuration::updateValue('productcomments_securekey', strtoupper(Tools::passwdGen(16)));		
	...
	public function install($keep = true)
	{
		...
		if (parent::install() == false ||
			...
			!Configuration::updateValue('PRODUCT_COMMENTS_FORM_SERVICE', 7) ||
			!Configuration::updateValue('PRODUCT_COMMENTS_FORM_PRODUCTS', 15) ||
	...
	public function uninstall($keep = true)
	{
		if (!parent::uninstall() || ($keep && !$this->deleteTables()) ||
			...
			!Configuration::deleteByName('PRODUCT_COMMENTS_FORM_SERVICE') ||
			!Configuration::deleteByName('PRODUCT_COMMENTS_FORM_PRODUCTS') ||
	...
	protected function _postProcess()
	{
		...
		{
			Configuration::updateValue('PRODUCT_COMMENTS_FORM_SERVICE', (int)Tools::getValue('PRODUCT_COMMENTS_FORM_SERVICE'));
			Configuration::updateValue('PRODUCT_COMMENTS_FORM_PRODUCTS', (int)Tools::getValue('PRODUCT_COMMENTS_FORM_PRODUCTS'));
		...
		elseif (Tools::isSubmit('submitNewComment'))
		{
			...
			if (!Tools::getValue('comentario') || !Validate::isMessage(Tools::getValue('comentario')))
				$errors[] = $this->l('Comment is incorrect', 'default');
			if (!Tools::isSubmit('customer_name') || !Tools::getValue('customer_name') || !Validate::isGenericName(Tools::getValue('customer_name')))
			...
			if (!count($errors))
			{
				...
				if ($product_comment_criterion->id)
					$product_comment_criterion->addGrade($comment->id, Tools::getValue('grade'));
			...
			if ($result)
				Tools::redirectAdmin(Context::getContext()->link->getAdminLink('AdminComentarios').'&configure='.$this->name.'&conf=4');
			else
				$this->_html .= '<div class="conf confirm alert alert-danger">'.$this->l('Error guardando comentario').'</div>';
		...
		elseif (Tools::isSubmit('statusproductcommentscriterion'))
		{
			...
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminComentarios').'&configure='.$this->name.'&tab_module='.$this->tab.'&conf=4&module_name='.$this->name);
		}
	...
	public function getContent()
	{
		...
		elseif (Tools::isSubmit('newcomment'))
			$this->_html .= $this->renderCommentForm();
		else
		{
			$this->_postProcess();
	...
	public function renderConfigForm()
	{
		$fields_form_1 = array(
			'form' => array(
				...
				'input' => array(
					...
					array(
						'type' => 'text',
						'label' => $this->l('Tiempo de envío del formulario de servicio'),
						'name' => 'PRODUCT_COMMENTS_FORM_SERVICE',
						'class' => 'fixed-width-xs',
						'suffix' => 'días',
					),	
					array(
						'type' => 'text',
						'label' => $this->l('Tiempo de envío del formulario de productos'),
						'name' => 'PRODUCT_COMMENTS_FORM_PRODUCTS',
						'class' => 'fixed-width-xs',
						'suffix' => 'días',
					),										
				),
		...
		$helper->currentIndex = $this->context->link->getAdminLink('AdminComentarios', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
		$helper->token = Tools::getAdminTokenLite('AdminComentarios');
	...
	public function renderModerateLists()
	{
		...
		{
			$comentaris = ProductComment::getByValidate(0, false);

			$comments = array();

			foreach($comentaris AS $k => $camp) {
            	foreach($camp AS $index => $valor) {
            		if($index == 'name' && $valor == '') {
            			$comments[$k][$index] = $this->l('SERVICIO');
            		} else {
            			$comments[$k][$index] = $valor;
            		}
            	}
		...
		$comentaris2 = ProductComment::getReportedComments();

		foreach($comentaris2 AS $k2 => $camp2) {
        	foreach($camp2 AS $index2 => $valor2) {
        		if($index2 == 'name' && $valor2 == '') {
        			$comments[$k2][$index2] = $this->l('SERVICIO');
        		} else {
        			$comments[$k2][$index2] = $valor2;
        		}
        	}
	...
	public function renderCriterionList()
	{
		...
		$helper->toolbar_btn['new'] = array(
			'href' => $this->context->link->getAdminLink('AdminComentarios').'&configure='.$this->name.'&module_name='.$this->name.'&updateproductcommentscriterion',
	...
	public function renderCommentsList()
	{
		...
		$helper->show_toolbar = true;
		$helper->toolbar_btn['new'] = array(
			'href' => $this->context->link->getAdminLink('AdminComentarios').'&configure='.$this->name.'&module_name='.$this->name.'&newcomment',
			'desc' => $this->l('Añadir Nuevo Comentario', null, null, false)
		);
		$helper->module = $this;
		$helper->listTotal = count($comments);
		$helper->identifier = 'id_product_comment';
	...
	public function getConfigFieldsValues()
	{
		return array(
			...
			'PRODUCT_COMMENTS_FORM_SERVICE' => Tools::getValue('PRODUCT_COMMENTS_FORM_SERVICE', Configuration::get('PRODUCT_COMMENTS_FORM_SERVICE')),
			'PRODUCT_COMMENTS_FORM_PRODUCTS' => Tools::getValue('PRODUCT_COMMENTS_FORM_PRODUCTS', Configuration::get('PRODUCT_COMMENTS_FORM_PRODUCTS')),
		);
	}

	public function getCommentFieldsValues()
	{
		$comment = new ProductComment();
	...
	function renderCommentForm()
	{
		...
		*/
		...
		foreach ($product_table_values as $key => $value)
		{
		...
		$fields_form_1 = array(
			'form' => array(
				'legend' => array(
					'title' => $this->l('Añadir nuevo comentario'),
					'icon' => 'icon-cogs'
				),
				'input' => array(
					array(
					...
					array(
						'type' => 'text',
						'lang' => false,
						'label' => $this->l('Clasificación (0 - 5)'),
						'name' => 'grade',
					),					
				),
			'submit' => array(
		...
		$helper->allow_employee_form_lang = Configuration::get('PS_BO_ALLOW_EMPLOYEE_FORM_LANG') ? Configuration::get('PS_BO_ALLOW_EMPLOYEE_FORM_LANG') : 0;
		$helper->identifier = $this->identifier;
		$helper->submit_action = 'submitNewComment';
		$helper->currentIndex = $this->context->link->getAdminLink('AdminComentarios', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
		$helper->token = Tools::getAdminTokenLite('AdminComentarios');
		$helper->tpl_vars = array(
			'fields_value' => $this->getCommentFieldsValues(),			
			'languages' => $this->context->controller->getLanguages(),
			'id_language' => $this->context->language->id
		);
	...
	public function displayApproveLink($token = null, $id, $name = null)
	{
		$this->smarty->assign(array(
			'href' => $this->context->link->getAdminLink('AdminComentarios').'&configure='.$this->name.'&module_name='.$this->name.'&approveComment='.$id,
	...
	{
		$this->smarty->assign(array(
			'href' => $this->context->link->getAdminLink('AdminComentarios').'&configure='.$this->name.'&module_name='.$this->name.'&noabuseComment='.$id,
	...
	public function hookProductTab($params)
	{
     ...
     public function hookPreFooter($params) {
     ...
     public function hookDisplayTopColumn($params) {
		...
		require_once(dirname(__FILE__).'/ProductComment.php');
		$average = ProductComment::getAverageGrade(0);
		//var_dump($average);
		$this->context->smarty->assign(array(
			'totalpuntuacio' => round($average['grade'])
		));

		return $this->display(__FILE__, 'widget_home.tpl');
     ...
     public function hookDisplayHome($params) {
		...
		$comments = ProductComment::getByProduct(0, 1, 12, null, true, true);

		$this->context->smarty->assign(array(
			'comments' => $comments
		));

		return $this->display(__FILE__, 'opinions_destacat.tpl');
	...
	public function hookHeader()
	{
		...
		if (in_array($this->page_name, array('product', 'productscomparison', 'opinions', 'opiniones', 'module-productcomments-opinions')))
		{
	...
	function cronTask() {
		...
		$orders = Db::getInstance()->ExecuteS('
			SELECT `id_customer`, `id_lang`, `reference`, `invoice_date`, `envia_servicio`, `envia_productos` FROM `'._DB_PREFIX_.'orders` 
			WHERE `valid`=1 AND `invoice_date` != "0000-00-00 00:00:00" AND (`envia_servicio` = 0 OR `envia_productos` = 0);');
		...
		if($orders) {
			...
			foreach($orders AS $order) {
				...
				if($order['envia_servicio'] == 0 && strtotime(date('Y-m-d H:i:s')) >= strtotime ( '+'.Configuration::get('PRODUCT_COMMENTS_FORM_SERVICE').' day' , strtotime ( $order['invoice_date'] ) )) {

					$enviament = 'servicio';

				} elseif ($order['envia_productos'] == 0 && strtotime(date('Y-m-d H:i:s')) >= strtotime ( '+'.Configuration::get('PRODUCT_COMMENTS_FORM_PRODUCTS').' day' , strtotime ( $order['invoice_date'] ) )) {

					$enviament = 'productos';

				}

				if($enviament == 'servicio' || $enviament == 'productos') {
					...
					if($customer) {
						$data = array(
							...
							'{link}' => $link->getModuleLink('productcomments', 'opinions', array(), true).'?id='.$order['reference'].'&tipo='.$enviament,
						...
						if(Mail::Send(
							...
							$this->name.'-'.$enviament,
							$this->l('Opinión cliente:').' '.$enviament,
							$data,
							$customer->email,
							...
							dirname(__FILE__).'/mails/')) 
						{
							...
							Db::getInstance()->Execute('
								UPDATE `'._DB_PREFIX_.'orders` SET `envia_'.$enviament.'` = 1 
								WHERE `reference` = "'.$order['reference'].'" AND `id_customer` = '.$customer->id); 

						} else 
							echo 'error enviament mail';
