	...
	<form method="post" action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'}" id="paypal_configuration">
		...
		<div class="box disabled" id="credentials">
			...
			<div class="paypal-hide" id="configuration">
				...
				<p>{l s='Confirmation page' mod='paypal'}</p>
				<p class="description">{l s='Choose order-confirmation if you want the customer to confirm the order again when he returns from PayPal.' mod='paypal'}</p>
				<input type="radio" name="paypal_confirmation" id="paypal_confirmation_0" value="0" {if $PayPal_confirmation == 0}checked="checked"{/if} /> <label for="paypal_confirmation_0">{l s='Direct (without confirmation, default)' mod='paypal'}</label><br />
				<input type="radio" name="paypal_confirmation" id="paypal_confirmation_1" value="1" {if $PayPal_confirmation == 1}checked="checked"{/if} /> <label for="paypal_confirmation_1">{l s='Order-confirmation' mod='paypal'}</label>
				<br/>
				
				<p>{l s='Default payment method' mod='paypal'}</p>
				<p class="description">{l s='Choose the default payment method in PayPal page' mod='paypal'}</p>
				<input type="radio" name="paypal_method" id="paypal_method_default" value="0" {if $PayPal_method == 0}checked="checked"{/if} /> <label for="paypal_method_default">{l s='PayPal account (default)' mod='paypal'}</label><br />
				<input type="radio" name="paypal_method" id="paypal_method_card" value="1" {if $PayPal_method == 1}checked="checked"{/if} /> <label for="paypal_method_card">{l s='Credit card' mod='paypal'}</label>

				<br /><br />
			</div>

			<hr>
			<div class="box" style="margin-left: 0px; padding-left: 0px;">
				<span class="paypal-section">4</span> <h3 class="inline">{l s='Paypal Fee' mod='paypal'}</h3>
				<br /><br />
				<dl>
					<dt><label for="fee">{l s='Flat Fee' mod='paypal'} : </label></dt>
					<dd><input type="number" step="any" name="fee" id="fee" value="{$PayPal_fee}" autocomplete="off" />
                    	<p class="clear">{l s='Use dot for decimal values' mod='paypal'}</p>
                    </dd>
					<dt><label for="pctfee">{l s='Percentage Share' mod='paypal'} : </label></dt>
                    <dd><input type="number" step="any" name="pctfee" id="pctfee" value="{$PayPal_pctfee}" autocomplete="off" />
                   		<p class="clear">{l s='Use dot for decimal values' mod='paypal'}</p>
                    </dd>
					<dt><label for="tax_rules[]">{l s='Tax Rule' mod='paypal'} : </label></dt>
					<dd>
						<select name="tax_rules[]">
							{html_options options=$taxes selected=$PayPal_tax}
						</select>
						<p class="clear">{l s='Tax that it is incluided on fee' mod='paypal'}</p>
					</dd>
					<dt><label for="feedetail">{l s='Fee Details' mod='paypal'} : </label></dt>
                    <dd>
						<input type="checkbox" name="feedetail" id="feedetail" value="1" {if $fee_detail == 1}checked="checked"{/if} /> <label for="feedetail">{l s='Display details (percent amount, flat fee, etc)' mod='paypal'}</label><br />
                   		<p class="clear">{l s='If not checked, total amount of fee will be displayed (required)' mod='paypal'}</p>
                    </dd>
				</dl>
				<div class="clear"></div>
			</div>
