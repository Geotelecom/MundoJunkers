	...
	<tr>
		...
		<td style="width: 80%">
			...
			<table style="width: 70%" >
				...
				<tr style="line-height:6px;background-color:{cycle values='#FFF,#DDD'};">
					<td style="width: 30%">{l s='Administration fees' pdf='true'}</td>
					<td style="width: 20%; text-align: right;">{$order->payment_fee_rate} %</td>
					{if !$use_one_after_another_method}
					<td style="width: 20%; text-align: right;">
					...
					{/if}
					<td style="width: 20%; text-align: right;">{if isset($is_order_slip) && $is_order_slip}- {/if}{displayPrice currency=$order->id_currency price=($order->payment_fee * ($order->payment_fee_rate/100) / (1+($order->payment_fee_rate/100)))}</td>
				</tr>
				{/if}
