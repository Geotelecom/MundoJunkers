...
if ($request_type && $ppec->type)
{
	...
	{
		if(isset($ppec->logs[2]) && strpos($ppec->logs[2],'10002'))
		{
			$aw_error = $ppec->display_aw_error();
		}
		$ppec->displayPayPalAPIError($ppec->l('Error during the preparation of the Express Checkout payment'), $ppec->logs, $aw_error);
	}
}
	...
	if ($ppec->hasSucceedRequest() && !empty($ppec->token))
	{
		...
		{//If address does not exists, we create it
			$address = setCustomerAddress($ppec, $customer);
                        if($address->id_state != 0 || !empty($address->id_state))
                            $address->add();
		}
