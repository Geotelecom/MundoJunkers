...
class PayPalIPN extends PayPal
{
	...
	public function checkPayment($payment_status, $mc_gross_not_rounded, $new_order)
	{
		...
		// Obtenemos el fee y si es > 0, se lo sumamos al total_price para la comparacion
		$fee = $this->getCost(Context::getContext()->cart);
		if($fee > 0)
			$total_price = Tools::ps_round($shipping + $subtotal + $tax + $fee, $this->decimals);
		else
		...
		{
			// Si le hemos sumado el fee, se lo restamos ya que al validarlo despues lo vuelve a sumar. Ya hemos pasado la comparacion
			if($fee > 0)
				$total_price = Tools::ps_round($shipping + $subtotal + $tax, $this->decimals);
