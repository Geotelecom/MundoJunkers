	...
	if ($firma_local === $firma_remota
		...
		if ($accesoDesde === 'POST') {
			...
			if ($cart->id_customer == 0
				...
				|| !$redsys->active) {
				escribirLog($idLog." -- "."ERROR: Cart {$pedido}({$pedidoSecuencial}) not loaded at line 101\n
			...
			if (!Validate::isLoadedObject($customer)) {
				escribirLog($idLog." -- "."ERROR: \$customer NOT loaded at line 107",$logActivo);
				Tools::redirect('index.php?controller=order&step=1');
			}

			/** VALIDACIONES DE DATOS y LIBRERÍA **/
			//Total
			$totalBoth = $cart->getOrderTotal(true, Cart::BOTH);
			$descuento = $cart->getOrderTotal(true, Cart::ONLY_DISCOUNTS);
			$totalCart = $totalBoth - $descuento;
			...
			/*try {
				...
				escribirLog($idLog." -- "."Excepción recuperando pedido " . $pedido . ": ".$e->getMessage(),$logActivo);
			}*/
			...
			if ($respuesta < 101) {
				/** Compra válida **/
				escribirLog($idLog." -- "."ERROR: access from GET at line 163",$logActivo);
				Tools::redirect('index.php?controller=order&step=1');
			} else {
				escribirLog($idLog." -- "."ERROR: ccess from GET at line 166",$logActivo);
				Tools::redirect('index.php?controller=order&step=1');
			}
		}
	} else {
		if ($accesoDesde === 'POST') {
			...
			escribirLog($idLog." -- "."ERROR: la firma no es correcta",$logActivo);
			Tools::redirect('index.php?controller=order&step=1');
		}
	}
}
