...
class Redsys extends PaymentModule
{
	...
	public function install()
	{
		...
		if (!parent::install()
			...
			return false;
		if (!Db::getInstance()->Execute("CREATE TABLE IF NOT EXISTS " ._DB_PREFIX_. "redsys (idPedido BIGINT NOT NULL, total BIGINT NOT NULL, PRIMARY KEY (idPedido)) ENGINE="._MYSQL_ENGINE_."  DEFAULT CHARSET=utf8;"))
			return false;			
		return true;
	}
	...
	public function hookPayment($params)
	{
		...
		try {
			escribirLog("INSERT INTO "._DB_PREFIX_."redsys (idPedido, total) VALUES(".$numpedido.", ".$cantidad.")", $logActivo);
			Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."redsys (idPedido, total) VALUES(".$numpedido.", ".$cantidad.")");
		} catch (Exception $e) {
			try {
				Db::getInstance()->Execute("CREATE TABLE IF NOT EXISTS " ._DB_PREFIX_. "redsys (idPedido BIGINT NOT NULL, total BIGINT NOT NULL, PRIMARY KEY (idPedido)) ENGINE="._MYSQL_ENGINE_."  DEFAULT CHARSET=utf8;");
				Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."redsys (idPedido, total) VALUES(".$numpedido.", ".$cantidad.")");
			} catch (Exception $e) {
				escribirLog("Excepción creando tabla redsys " . $numpedido . ": ". $e->getMessage(), $logActivo);
			}
			escribirLog("Excepción insertando pedido " . $numpedido . ": ". $e->getMessage(), $logActivo);
		}
		...
		$miObj->setParameter("Ds_Merchant_Module","prestashop_redsys_2.8.5");
