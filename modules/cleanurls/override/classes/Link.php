...
class Link extends LinkCore
{
    ...
    */
    public function getCategoryLink($category, $alias = null, $id_lang = null, $selected_filters = null, $id_shop = null)
	{
		...
		$url = 'https://'.$shop->domain.$shop->getBaseURI().$this->getLangLink($id_lang, null, $id_shop);
		...
		if(is_object($category->meta_keywords) || is_array($category->meta_keywords))
			$params['meta_keywords'] = Tools::str2url($category->meta_keywords[$id_lang]);
		elseif ($category->meta_keywords != "")
			$params['meta_keywords'] = Tools::str2url($category->meta_keywords);

		if(is_object($category->meta_title) || is_array($category->meta_title))
			$params['meta_title'] = Tools::str2url($category->meta_title[$id_lang]);
		elseif ($category->meta_title != "")
