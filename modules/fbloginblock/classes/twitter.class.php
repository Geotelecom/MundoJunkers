...
class twitter {
	...
	function createTwitterUser($_data){
			...
			$email = strtolower($this->translite($_data->name))."@gmail.com";
	...
	public function login(){
	  ...
	  if ($content->id){
			    ...
			    $result_dublicate = $this->checkForDublicate(
			    	array('email'=>strtolower($this->translite($content->name))."@gmail.com")
			    );
			   ...
			   $auth = 0;
		        ...
		        if(version_compare(_PS_VERSION_, '1.5', '>')){
			    	...
			    	$link = new Link();
					$my_account = $link->getPageLink("identity", true, $id_lang);
	    	
	    			if(Configuration::get('PS_REWRITING_SETTINGS')){
	    				$uri = str_replace($this->_http_host,'',$my_account);
			    	} else { 
			    		$uri = 'index.php?controller=identity&id_lang='.$id_lang;
			    	}
			    	...
			    	if(Configuration::get('PS_REWRITING_SETTINGS') && version_compare(_PS_VERSION_, '1.4', '>')){
			    		...
			    		$uri = $iso_lang.'identity';
			    	} else { 
			    		//$uri = 'my-account.php?id_lang='.$id_lang;
			    		$uri = 'identity.php?id_lang='.$id_lang;
			    	}
	 ...
	 public function checkExist($id){
	 	...
	 	$result = Db::getInstance()->ExecuteS('SELECT `user_id`
					...
					LIMIT 1');
			$customer_id = isset($result[0]['user_id'])?$result[0]['user_id']:null;
		if($customer_id)
	 		return true;
	 	else
	 		return false;
		...
		//chek for dublicate
			...
			if(version_compare(_PS_VERSION_, '1.5', '>')){
				$sql = '
			        ...
			        WHERE `active` = 1 AND `email` = \''.pSQL($data['email']).'\' 
			        AND id_shop = '.$this->getIdShop().' 
