...
{if $fbloginblock_rightcolumnf == "rightcolumnf" || $fbloginblock_rightcolumng == "rightcolumng" 
	|| $fbloginblock_rightcolumnp == "rightcolumnp" || $fbloginblock_rightcolumnt == "rightcolumnt" 
	|| $fbloginblock_rightcolumny == "rightcolumny" || $fbloginblock_rightcolumnl == "rightcolumnl"
	|| $fbloginblock_rightcolumnm == "rightcolumnm"}
...
<form action="{$base_dir_ssl}{if $fbloginblockis_rewrite == 1}{$fbloginblockiso_lang}{if $fbloginblockis15 == 1}login{else}authentication{/if}{else}authentication.php{/if}" method="post">
		<fieldset class="block_content" style="border-bottom:1px none #D0D3D8;border-top:none">
			...
			<p class="submit">
				...
				{/if}
	 			...
	 			<a href="javascript:void(0)" title="Google" 
		   		   onclick="javascript:popupWin = window.open('{$base_dir}modules/fbloginblock/login.php{if $fbloginblockorder_page == 1}?p=google&http_referer={$fbloginblockhttp_referer|urlencode}{/if}', 'openId', 'location,width=512,height=512,top=0');popupWin.focus();">
				...
				<a href="javascript:void(0)" title="Yahoo" 
	 			...
	 			{/if}
				{if $fbloginblock_rightcolumnp == "rightcolumnp" && $fbloginblockp_on == 1}
			...
			<p class="lost_password" style="margin-top:10px;padding-left:0px">
				<a href="{$base_dir}{if $fbloginblockis_rewrite == 1}{$fbloginblockiso_lang}password-recovery{else}password.php{/if}">{l s='Forgot your password?' mod='fbloginblock'}</a>
			</p>
		</fieldset>
</form>
</div>
...
<div class="block">
		...
		<div>
		 ...
		 <br/> 
		<div>
			<img src="{$img_dir}icon/cart.gif" alt="{l s='Cart' mod='fbloginblock'}"/>
			<a href="{$base_dir_ssl}{if $fbloginblockis_rewrite == 1}{$fbloginblockiso_lang}order{else}order.php{/if}" title="{l s='Cart' mod='fbloginblock'}"><b>{l s='Cart:' mod='fbloginblock'}</b></a>
