	   ...
	   if (Tools::isSubmit('gfromloaddefault')){
           ...
           if($fields){
                ...
                foreach($languages as $language){
                    ...
                    $results['datassendersubject'][$language['id_lang']] = '['.$shopname.'] '.$this->l('Your message has been successfully sent');
                }
                $results['errors'] = 0;
           }
           ...
           if($fields !='' && $fields !=','){
                ...
                foreach($languages as $language){
                    ...
                    $results['fieldsData'] = gformbuilderprofieldsModel::getAllFields2($fields,(int)$language['id_lang'],$id_shop);
                }
                $results['errors'] = 0;
           }
	       die(Tools::jsonEncode($results));
           ...
           if (Tools::isSubmit('submitAddgformbuilderpro')){
               if(is_object($return) && get_class($return) == 'gformbuilderproModel'){
                    ...
                    if($savenew){
                        ...
                        $shopname=Tools::safeOutput(Configuration::get('PS_SHOP_NAME', null, null, $id_shop));
                        foreach($languages as $language){
                            if(isset($formModelObj->emailtemplate) && isset($formModelObj->emailtemplatesender) && ($formModelObj->emailtemplate[(int)$language['id_lang']] == '' || $formObj->emailtemplatesender[(int)$language['id_lang']] == '')){
    ...
    public function renderForm()
    {
        ...
        if($id_gformbuilderpro>0){
           ...
           $this->fields_value = array(
            ...
            $languages = Language::getLanguages(false);
            foreach($languages as $language){
                $this->fields_value['submittitle'][$language['id_lang']] = $this->l('Sent');
        ...
        $input[] = array(
            ...
            'desc' => $this->l('To add "Hook" click in the field, write hook name(ex: displayHome), and then press "Enter".Learn more about Prestashop Front-office hook: ').'<a target="_blank" href="http://doc.prestashop.com/display/PS15/Hooks+in+PrestaShop+1.5">http://doc.prestashop.com/display/PS15/Hooks+in+PrestaShop+1.5</a>',
            'name' => 'hooks');
        $input[] = array(
