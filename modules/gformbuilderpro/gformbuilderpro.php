...
class Gformbuilderpro extends Module
{
    public function __construct()
    {
        ...
        parent::__construct();
        $this->displayName = $this->l("Form Builder Pro");
    ...
    public function hookConfigFieldAjax($params){
        ...
        if($params['typefield']){
            ...
            if(file_exists($shortcode_dir.$typefield.'.php')){
                ...
                $html_extra='';
                if(version_compare(_PS_VERSION_,'1.6') == -1){
                    $html_extra = '<button type="submit" class="btn btn-default btn btn-default pull-left" name="cancelShortcode"><i class="process-icon-cancel"></i>'.$this->l('Cancel').'</button>';
                }
