...
class RoySpecials extends Module
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Roy Specials block');
		$this->description = $this->l('Adds a block of your price discounts with countdowns');
	...
	public function hookRightColumn($params)
	{
		...
		if (!Configuration::get('BLOCKSPECIALS_NB_CACHES') || !$this->isCached('royspecials.tpl', $this->getCacheId('royspecials|'.$random)))
		{
			if (!($specials = Product::getPricesDrop((int)$params['cookie']->id_lang, 0, 20)) && !Configuration::get('PS_BLOCK_SPECIALS_DISPLAY'))
				return;
		...
		return $this->display(__FILE__, 'royspecials.tpl', (Configuration::get('BLOCKSPECIALS_NB_CACHES') ? $this->getCacheId('royspecials|'.$random) : null));
	}
	...
	public function hookHeader($params)
	{
		if (Configuration::get('PS_CATALOG_MODE'))
			return;
		$this->context->controller->addCSS(($this->_path).'css/royspecials.css', 'all');
	}
	...
	public function hookDisplayHomeTab($params)
	{
		...
		if (!$this->isCached('tab.tpl', $this->getCacheId('royspecials-tab')))
			RoySpecials::$cache_specials = Product::getPricesDrop((int)$params['cookie']->id_lang, 0, Configuration::get('BLOCKSPECIALS_SPECIALS_NBR'));

		if (RoySpecials::$cache_specials === false)
			return false;

		return $this->display(__FILE__, 'tab.tpl', $this->getCacheId('royspecials-tab'));
	}
	...
	public function hookDisplayHomeTabContent($params)
	{
		...
		if (!$this->isCached('royspecials-home.tpl', $this->getCacheId('royspecials-home')))
		{
			$this->smarty->assign(array(
				'specials' => RoySpecials::$cache_specials,
		...
		return $this->display(__FILE__, 'royspecials-home.tpl', $this->getCacheId('royspecials-home'));
	}
	...
	{
		parent::_clearCache('royspecials.tpl');
		parent::_clearCache('royspecials-home.tpl', 'royspecials-home');
		parent::_clearCache('tab.tpl', 'royspecials-tab');
	}
}
