...
class RoyBanners extends Module
{
	...
	public function __construct()
	{
		...
		parent::__construct();
		$this->displayName = $this->l('Roy banners');
		$this->description = $this->l('Put banners on top, bottom, left or right sections');
	...
	{
		return (
			Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'roybanners`') &&
			Db::getInstance()->Execute('
			CREATE TABLE `'._DB_PREFIX_.'roybanners` (
	...
	protected function installFixture($hook, $id_image, $id_shop, $id_lang)
	{
		...
		$result &= Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'roybanners` ( 
	...
	{
		$images = Db::getInstance()->executeS('SELECT image FROM `'._DB_PREFIX_.'roybanners`');
		foreach ($images as $image)
			$this->deleteImage($image['image']);

		if (!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'roybanners`') || !parent::uninstall())
			return false;
	...
	public function hookdisplaySubTopColumn()
	{
	...
	protected function removeItem()
	{
		...
		if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'roybanners` WHERE id_item = '.(int)$id_item))
			$this->deleteImage($image);

		Db::getInstance()->delete(_DB_PREFIX_.'roybanners', 'id_item = '.(int)$id_item);

		if (Db::getInstance()->Affected_Rows() == 1)
		{
			Db::getInstance()->execute('
				UPDATE `'._DB_PREFIX_.'roybanners` 
			...
			Tools::redirectAdmin('index.php?controller=AdminBanners&token='.Tools::getAdminTokenLite('AdminBanners'));
		}
		else
	...
	protected function updateItem()
	{
		...
		{
			if ($old_image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'roybanners` WHERE id_item = '.(int)$id_item))
		...
		if (!Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'roybanners` SET 
					title = \''.pSQL($title).'\',
		...
		{
			if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'roybanners` WHERE id_item = '.(int)Tools::getValue('item_id')))
				$this->deleteImage($image);
	...
	protected function renderThemeConfiguratorForm()
	{
		...
		foreach ($this->languages as $language)
		{
			...
			foreach ($hooks[$language['id_lang']] as $hook)
				$items[$language['id_lang']][$hook] = Db::getInstance()->ExecuteS('
					SELECT * FROM `'._DB_PREFIX_.'roybanners` 
		...
		$this->context->smarty->assign('htmlitems', array(
			...
			'postAction' => 'index.php?controller=AdminBanners&token='.Tools::getAdminTokenLite('AdminBanners'),
			'id_shop' => $id_shop
		));
